...
async def cmd_start(message: types.Message):
    await message.answer_sticker('CAACAgIAAxkBAAMVZKVO9Twjk_6m39R8Nm50KHkWJhsAAoUAA8GcYAyLjB0fSFNdIi8E')
    await message.answer(f'Welcome to the club, {message.from_user.first_name}!', reply_markup=main)

    if message.from_user.id == int(os.getenv('ADMIN_ID')):
        await message.answer(f'You entered admin mode', reply_markup=main_admin)
...
async def get_donation_info(message: types.Message):
    if message.from_user.id == int(os.getenv('ADMIN_ID')):
        await message.answer(f'You entered admin mode', reply_markup=admin_panel)
    else:
        await message.answer_sticker('CAACAgIAAxkBAAMiZKVStZZorGto_KnKzvCGd0-nerQAAhYAAzed7hJaF5ONGLk2Qi8E')
